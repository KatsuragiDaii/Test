...
function SalePageContent() {
  ...
  useEffect(() => {
    const fetchSaleProducts = async () => {
      setIsLoading(true);
      try {
        // Ambil seluruh produk yang isi kolom 'discount'-nya tidak kosong (sedang Sale)
        ...
        console.error("Gagal memuat katalog diskon:", err);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const filteredProducts = products.filter((product) => {
    const matchesGender = selectedGenders.length === 0 || 
      ...
      (product.gender === "Unisex" && (selectedGenders.includes("Men") || selectedGenders.includes("Women")));
  ...
  if (isLoading) {
    return (
      <div className="w-full min-h-screen bg-white flex flex-col items-center justify-center gap-4 text-[10px] font-bold tracking-widest text-neutral-400 uppercase select-none">
        ...
        <span>Loading Sale Catalog...</span>
      </div>
    );
  }
  ...
  return (
    <div className="w-full min-h-screen bg-white">
      ...
      <div className="w-full bg-[#F5F5F5] pt-14 pb-10 px-6 lg:px-12 border-b border-neutral-200 flex flex-col gap-1 uppercase select-none">
        <h1 className="text-2xl lg:text-3xl font-black tracking-[0.2em] text-neutral-950">Sale</h1>
      ...
      <div className="max-w-full w-full mx-auto px-4 md:px-10 lg:px-16 h-16 flex items-center justify-between lg:justify-end pt-4">
        <button className="flex items-center gap-2 lg:hidden text-[11px] font-bold tracking-wider border border-neutral-200 px-4 py-2 bg-white text-gray-900 rounded-xl shadow-2xs uppercase">
          <SlidersHorizontal size={13} /> Filter
        </button>

        <div className="relative z-30">
          <button onClick={() => setIsSortOpen(!isSortOpen)} className="border border-neutral-300 bg-white px-4 py-2 flex items-center justify-between gap-6 text-[13px] text-neutral-900 font-normal hover:bg-neutral-50 transition-colors duration-150 cursor-pointer min-w-[150px] text-left select-none rounded-xl shadow-2xs">
            <span>{sortBy}</span>
            <ChevronDown size={14} className={`text-gray-600 transition-transform duration-200 ${isSortOpen ? 'rotate-180' : ''}`} />
          </button>
      ...
      <div className="max-w-full w-full mx-auto px-4 md:px-10 lg:px-16 py-6 flex flex-col lg:flex-row gap-10 lg:gap-14">
        ...
        <aside className="hidden lg:block w-56 shrink-0 space-y-8 pt-2">
          <div className="border-b border-neutral-100 pb-6">
            <button onClick={() => toggleSection('gender')} className="w-full flex items-center justify-between text-[11px] font-bold tracking-widest uppercase text-neutral-950 mb-4 cursor-pointer select-none">
              Gender {openSections.gender ? <span className="text-sm font-light">—</span> : <span className="text-sm font-light">+</span>}
            </button>
            {openSections.gender && (
              <div className="space-y-3 text-[11px] tracking-wider text-neutral-600 font-bold uppercase">
                {['Men', 'Women'].map((item) => (
                  <label key={item} className="flex items-center gap-3 cursor-pointer group hover:text-black select-none">
                    <input type="checkbox" className="w-3.5 h-3.5 accent-black rounded-md cursor-pointer border-neutral-300" checked={selectedGenders.includes(item)} onChange={() => handleGenderChange(item)} />
                    <span>{item}</span>
                  </label>
                ))}
              </div>
          ...
          <div className="border-b border-neutral-100 pb-6">
            <button onClick={() => toggleSection('productType')} className="w-full flex items-center justify-between text-[11px] font-bold tracking-widest uppercase text-neutral-950 mb-4 cursor-pointer select-none">
              Product Type {openSections.productType ? <span className="text-sm font-light">—</span> : <span className="text-sm font-light">+</span>}
            </button>
        ...
        <main className="flex-1">
          {sortedProducts.length > 0 ? (
              ...
              {sortedProducts.map((product) => (
                <Link href={`/products/${product.id}`} key={product.id} className="group cursor-pointer flex flex-col relative bg-white pb-4 block transition-all">
                  <div className="w-full aspect-[3/4] bg-[#f9f9f9] border border-neutral-100 relative flex items-center justify-center overflow-hidden transition-all duration-500 group-hover:bg-[#f0f0f0] rounded-xl shadow-xs">
                    ...
                    <img src={product.images && product.images.length > 0 ? product.images[0] : 'https://images.unsplash.com/photo-1603487742131-4160ec999306?q=80&w=1000&auto=format&fit=crop'} alt={product.name} className="w-full h-full object-cover object-top" />
                  </div>
                  ...
                  <div className="mt-4 space-y-1 px-1 text-left">
                    ...
                    <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[10px] font-medium tracking-wider">
                      <span className="text-red-600 font-black font-mono">Rp {Number(product.price).toLocaleString('id-ID')}</span>
            ...
            <div className="w-full text-center py-24 text-xs font-bold tracking-widest text-neutral-400 uppercase">Tidak ada produk diskon yang cocok dengan filter.</div>
          )}
        </main>
...
export default function SalePage() {
  return (
    <Suspense fallback={<div className="w-full min-h-screen bg-white flex items-center justify-center text-xs font-mono tracking-widest text-neutral-400 uppercase select-none">Loading Sale Catalog Shell...</div>}><SalePageContent /></Suspense>
  );
}